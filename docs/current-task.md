...
- [x] Remove FLECS include from renderer — `src/render/renderer.cpp` line 5 includes `<flecs.h>`, violating module boundaries. Fix: pass `SimConfig*` and `SimulationState*` through `RenderState` (in `include/render_state.h`). Populate in `register_render_sync_system()` (in `src/ecs/systems.cpp`). Then remove `<flecs.h>` and the `flecs::world*` cast from renderer.cpp. The `void* world_ptr` parameter can become unused or removed.
